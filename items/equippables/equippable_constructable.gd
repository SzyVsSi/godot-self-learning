...
func check_build_validity() -> bool:
  if item_place_ray_cast.is_colliding():
    constructable_area.global_position = item_place_ray_cast.get_collision_point()
  ...
  constructable_area.global_position = item_place_ray_cast.to_global(item_place_ray_cast.target_position)
  return false
...
func try_to_construct() -> void:
  ...
  set_process(false)
  EventSystem.SPA_spawn_scene.emit(ItemConfig.get_constructable_scene(constructable_item_key), constructable_area.global_transform, true)
