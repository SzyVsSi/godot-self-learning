...
func equip_item(key: ItemConfig.Keys) -> void:
	unequip_item()
	var item_scene := ItemConfig.get_equippable_item(key).instantiate()
	...
	if item_scene is EquippableConstructable:
		item_scene.constructable_item_key = key
